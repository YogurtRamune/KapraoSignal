...
- Supports connecting with the last fired value (`Signal:ConnectWithLast()` and `Signal:ImmediateConnectWithLast()`)
...
```lua
Signal:ImmediateConnectWithLast(callback: (...any) -> ()) -> Connection?

```

Acts exactly like `Signal:ImmediateConnect()`, but if `Signal.EverFired` is true, the callback will be invoked **synchronously** with `Signal.LastestFiredValue` immediately after connecting.
...
```lua
Signal:ConnectWithLast(callback: (...any) -> ()) -> Connection?

```

Acts exactly like `Signal:Connect()`, but if `Signal.EverFired` is true, the callback will be spawned **asynchronously** (via `task.spawn`) with `Signal.LastestFiredValue` immediately after connecting.
